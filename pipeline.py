...
class Pipeline:
        ...
        if os.path.exists(path) and option == 'existing':
            ...
            if self.language is 'c':
                ...
                def parse_program(func):
                    ...
                    return tree
                source = pd.read_csv(self.root+self.language+'/bcb_funcs_all2.tsv', sep='\t', header=None, encoding='utf-8')
    ...
    def split_data(self):
        ...
        none_clone_data = data[data['label'].isin([0])]
        clone_data = data[data['label'].isin([1, 2, 3, 4, 5])]

        none_clone_num = len(none_clone_data)
        clone_num = len(clone_data)

        ratios = [int(r) for r in self.ratio.split(':')]

        none_train_split = int(ratios[0] / sum(ratios) * none_clone_num)
        none_val_split = none_train_split + int(ratios[1] / sum(ratios) * none_clone_num)

        train_split = int(ratios[0] / sum(ratios) * clone_num)
        val_split = train_split + int(ratios[1] / sum(ratios) * clone_num)

        none_clone_data = none_clone_data.sample(frac=1, random_state=666)
        clone_data = clone_data.sample(frac=1, random_state=666)

        none_clone_train = none_clone_data.iloc[:none_train_split]
        none_clone_dev = none_clone_data.iloc[none_train_split:none_val_split]
        none_clone_test = none_clone_data.iloc[none_val_split:]

        clone_train = clone_data.iloc[:train_split]
        clone_dev = clone_data.iloc[train_split:val_split]
        clone_test = clone_data.iloc[val_split:]

        train = pd.concat([none_clone_train, clone_train])
        dev = pd.concat([none_clone_dev, clone_dev])
        test = pd.concat([none_clone_test, clone_test])
    ...
    def run(self):
        ...
        if self.language is 'c':
            ...
            self.read_pairs('bcb_pair_ids2.pkl')
        print('split data...')
        self.split_data()
